...
class H1Flat(BaseTask):
    ...
    def post_physics_step(self):
        ...
        self.feet_ground_time *= self.contact_filt
    ...
    def _reset_root_states(self, env_ids):
        ...
        if self.custom_origins:
            ...
            self.root_states[env_ids] = self.base_init_state
            self.root_states[env_ids, :3] += self.env_origins[env_ids]
        if self.cfg.init_state.randomize_rotation:
            random_yaws = torch_rand_float(-3.14159265359, 3.14159265359, (len(env_ids), 1), device=self.device)
            quats = torch.zeros(len(env_ids), 4, device=self.device)
            quats[:, 3] = torch.cos(random_yaws / 2).view(len(env_ids))
            quats[:, 2] = torch.sin(random_yaws / 2).view(len(env_ids))
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec[9:28] = noise_scales.dof_pos * noise_level * self.obs_scales.dof_pos
        noise_vec[28:47] = noise_scales.dof_vel * noise_level * self.obs_scales.dof_vel
        noise_vec[47:66] = 0. # previous actions
        return noise_vec
    ...
    def _reward_feet_air_time_positive_biped(self):
        # Reward long steps
        # Need to filter the contacts because the contact reporting of PhysX is unreliable on meshes
        in_contact = self.feet_ground_time > 0.0
        in_mode_time = torch.where(in_contact, self.feet_ground_time, self.feet_air_time)
        single_stance = torch.sum(in_contact.int(), dim=1) == 1
        reward = torch.min(torch.where(single_stance.unsqueeze(-1), in_mode_time, 0.0), dim=1)[0]
        reward = torch.clamp(reward, max=self.cfg.rewards.feet_air_time_threshold)
    ...
    def _reward_feet_body_direction(self):
        _, _, body_direction = euler_from_quat(self.base_quat)
        _, _, lfeet_direction = euler_from_quat(self.rigid_body_states[:, self.feet_indices[0], 3:7])
        _, _, rfeet_direction = euler_from_quat(self.rigid_body_states[:, self.feet_indices[1], 3:7])
        rew = torch.abs(body_direction - lfeet_direction) + torch.abs(body_direction - rfeet_direction)
        return rew
    
    def _reward_feet_body_position(self):
        feet_pos_global = self.rigid_body_states[:, self.feet_indices, :3]
        body_pos = self.root_states[:, :3].unsqueeze(1)
        relative_pos = feet_pos_global - body_pos
        left_feet_pos_body = quat_rotate_inverse(self.base_quat, relative_pos[:, 0, :])
        right_feet_pos_body = quat_rotate_inverse(self.base_quat, relative_pos[:, 1, :])
        alignment_reward = torch.abs(left_feet_pos_body[:,1] - 0.2029) + torch.abs(right_feet_pos_body[:,1] + 0.2029)
